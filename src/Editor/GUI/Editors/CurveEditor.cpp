...
#define tween$bounceout(p) ((p) < 4 / 11.0 ? (121 * (p) * (p)) / 16.0 : (p) < 8 / 11.0 ? (363 / 40.0 * (p) * (p)) - (99 / 10.0 * (p)) + 17 / 5.0 : (p) < 9 / 10.0 ? (4356 / 361.0 * (p) * (p)) - (35442 / 1805.0 * (p)) + 16061 / 1805.0 : (54 / 5.0 * (p) * (p)) - (513 / 25.0 * (p)) + 268 / 25.0)
...
int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points, int& item) {
    ...
    /// @todo Fix compatibility with new ImGui API.
    const bool hovered = false; // IsHovered(bb, id);
