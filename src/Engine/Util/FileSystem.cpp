...
std::vector<std::string> DirectoryContents(const std::string& directoryName, unsigned int type) {
    ...
    while (find) {
        if (((type & DIRECTORY && findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) || (type & FILE && !(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))) && strcmp(findFileData.cFileName, ".") && strcmp(findFileData.cFileName, "..")) {
    ...
    while ((entry = readdir(directory)) != NULL) {
        if (((type & DIRECTORY && entry->d_type == DT_DIR) || (type & FILE && entry->d_type != DT_DIR)) && strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
