...
namespace Physics {
TriggerObserver::TriggerObserver(btCollisionObject& body) : btCollisionWorld::ContactResultCallback(), rigidBody(body) {}
...
void TriggerObserver::PostIntersectionTest() {
    if (didCallback) {
        ...
        default: { assert(phase == IntersectionPhase::Retained); }
        }
    } else {
        ...
        default: { assert(phase == IntersectionPhase::None); }
        }
    }
...
btScalar TriggerObserver::addSingleResult(btManifoldPoint& cp, const btCollisionObjectWrapper* colObj0, int partId0, int index0, const btCollisionObjectWrapper* colObj1, int partId1, int index1) {
