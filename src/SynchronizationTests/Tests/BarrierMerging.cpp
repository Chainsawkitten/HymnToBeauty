...
#include "PostProcessingOffset.vert.hpp"
...
const char* BarrierMerging::GetName() const {
    return "Barrier merging";
}

void BarrierMerging::Setup(Video::LowLevelRenderer* renderer, const glm::uvec2& screenSize) {
    ...
    blurVertexShader = renderer->CreateShader(POSTPROCESSINGOFFSET_VERT, Shader::Type::VERTEX_SHADER);
...
void BarrierMerging::Frame() {
    ...
    Buffer* offsetBuffer = renderer->CreateTemporaryBuffer(Buffer::BufferUsage::UNIFORM_BUFFER, sizeof(glm::vec2), &offset);
    ...
    commandBuffer->BindUniformBuffer(ShaderProgram::BindingType::MATRICES, offsetBuffer);
