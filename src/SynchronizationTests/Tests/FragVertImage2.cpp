...
const char* FragVertImage2::GetName() const {
    return "Frag->Vert (Image) 2";
}

void FragVertImage2::Setup(Video::LowLevelRenderer* renderer, const glm::uvec2& screenSize) {
    this->renderer = renderer;
    this->screenSize = screenSize;

    mainRenderTarget = renderer->CreateRenderTarget(screenSize, Texture::Format::R8G8B8A8);
    unrelatedRenderTarget = renderer->CreateRenderTarget(screenSize, Texture::Format::R8G8B8A8);
    ...
    textureData = new unsigned char[textureSize.x * textureSize.y * 3];
    for (std::size_t i = 0; i < textureSize.x * textureSize.y; ++i) {
        textureData[i * 3 + 0] = i % 255;
        textureData[i * 3 + 1] = 0;
        textureData[i * 3 + 2] = 128;
    ...
    cubeInstanceBuffer = renderer->CreateBuffer(Buffer::BufferUsage::STORAGE_BUFFER, sizeof(PerInstance) * instanceCount, instances);

    delete[] instances;

    // Uniform buffer holding lighting data.
    uniformData.lightCount = 20;
    cubeUniformBuffer = renderer->CreateBuffer(Buffer::BufferUsage::UNIFORM_BUFFER, sizeof(uniformData), &uniformData);

    // Storage buffer holding lights.
    lights = new Light[uniformData.lightCount];
    for (uint32_t i = 0; i < uniformData.lightCount; ++i) {
        float x = static_cast<float>(i % 10) * 2.0f;
        float y = static_cast<float>(i / 10) * 2.0f;
    ...
    cubeLightBuffer = renderer->CreateBuffer(Buffer::BufferUsage::STORAGE_BUFFER, sizeof(Light) * uniformData.lightCount, lights);

    delete[] lights;
}

void FragVertImage2::Frame() {
    ...
    // Second render pass doesn't use the offset and can be rendered in parallel to the first.
    commandBuffer->BeginRenderPass(unrelatedRenderTarget, RenderPass::LoadOperation::CLEAR, depthBuffer);
    ...
    commandBuffer->BindUniformBuffer(ShaderProgram::BindingType::UNIFORMS, cubeUniformBuffer);
    commandBuffer->BindStorageBuffers({cubeInstanceBuffer, cubeLightBuffer});
    commandBuffer->DrawIndexedInstanced(cube->GetIndexCount(), cubeInstanceCount);
    commandBuffer->EndRenderPass();

    // Third render pass renders a plane.
    ...
    commandBuffer->BindMaterial({
        {unrelatedRenderTarget, renderer->GetSampler(Sampler::Filter::LINEAR, Sampler::Clamping::CLAMP_TO_EDGE)},
...
void FragVertImage2::Shutdown() {
    ...
    delete cube;
    delete cubeAlbedo;
    delete cubeInstanceBuffer;
    delete cubeUniformBuffer;
    delete cubeLightBuffer;
